...
public class PayBill extends JFrame implements ActionListener{
    ...
    PayBill(String meter){
        ...
        try{
            Conn c = new Conn();
            ResultSet rs = c.s.executeQuery("select * from customer_detailss where meter_no = '"+meter+"'");
            while(rs.next()){
                l11.setText(rs.getString("meter_no"));
                l12.setText(rs.getString("name"));
            }
            rs = c.s.executeQuery("select * from billing_infoe where meter_no = '"+meter+"' AND month = 'January' ");
            while(rs.next()){
        ...
        c1.addItemListener(new ItemListener(){
            ...
            public void itemStateChanged(ItemEvent ae){
                try{
                    Conn c = new Conn();
                    ResultSet rs = c.s.executeQuery("select * from billing_infoe where meter_no = '"+meter+"' AND month = '"+c1.getSelectedItem()+"'");
                    while(rs.next()){
                        l13.setText(rs.getString("units"));
                        l14.setText(rs.getString("total_bill"));
                        l15.setText(rs.getString("status"));
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource() == b1){
            try{
                Conn c = new Conn();
                c.s.executeUpdate("update billing_infoe set status = 'Paid' where meter_no = '"+meter+"' AND month = '"+c1.getSelectedItem()+"'");

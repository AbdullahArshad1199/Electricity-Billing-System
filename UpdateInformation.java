...
public class UpdateInformation extends JFrame implements ActionListener{
    ...
    UpdateInformation(String meter){
        ...
        try{
            Conn c = new Conn();
            ResultSet rs = c.s.executeQuery("select name,meter_no,address,city,state,email,phone from customer_detailss where meter_no = '"+meter+"'");
            while(rs.next()){
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource() == b1){
            ...
            try{
                Conn c = new Conn();
                c.s.executeUpdate("update customer_detailss set address = '"+s3+"', city = '"+s4+"', state = '"+s5+"', email = '"+s6+"', phone = '"+s7+"' where meter_no = '"+meter+"'");
